...
object Problems extends App{
  ...
  def combinations[T](n: Int, l: List[T]): List[List[T]] = {
    l.permutations.flatMap(x => {val(a,b)=x.splitAt(n);Set(a.toSet,b.toSet)}).filter(l => l.size == n).toSet.map( (s: Set[T]) => s.toList).toList
  }
  println("P026: " + combinations(3, List(1,2,3,4,5,6)))
  println("P026: " + combinations(3, List('a, 'b, 'c, 'd, 'e, 'f)))
  /**
  ...
  /**
    ...
    * But this time the objective is to sort the elements according to their length frequency; i.e. in the default, sorting is done ascendingly,
